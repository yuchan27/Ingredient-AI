...
gcloud run deploy foodlens-ai-api `
  ...
  --set-env-vars NODE_ENV=production,FIREBASE_PROJECT_ID=YOUR_PROJECT_ID,SMTP_HOST=smtp.gmail.com,SMTP_PORT=587,SMTP_SECURE=false,SMTP_USER=YOUR_GMAIL,SMTP_FROM_NAME="FoodLens AI",SMTP_FROM_EMAIL=YOUR_GMAIL `
...
已安裝的 APK 不必重新建置。登入後可在 App 的「設定 > API 主機」貼上 Cloud Run HTTPS URL，
App 會將此網址保存在裝置上並用於後續圖片分析。
